...
Respond with this exact JSON structure:
{
  ...
  `}function Gt(){X&&(document.querySelectorAll(".screen").forEach(e=>e.classList.remove("active")),document.getElementById("add").classList.add("active"),document.querySelectorAll(".nav-btn").forEach(e=>e.classList.remove("active")),document.querySelector('.nav-btn[onclick*="add"]').classList.add("active"),setTimeout(()=>{const e=document.getElementById("f-search");e&&(e.value=X.title,B(()=>Promise.resolve().then(()=>fo),void 0).then(t=>{Me?.predicted_scores&&t.prefillWithPrediction(Me.predicted_scores),t.liveSearch(X.title)}))},100))}let oe="all",Xe="focused",V=[],A=0,C={},L={},se=[];const Jt={focused:15,thorough:30,deep:50},qe=8;function Kt(e){oe=e,document.querySelectorAll('[id^="calcat_"]').forEach(t=>t.classList.remove("active")),document.getElementById("calcat_"+e).classList.add("active")}function Qt(e){Xe=e,document.querySelectorAll('[id^="calint_"]').forEach(t=>t.classList.remove("active")),document.getElementById("calint_"+e).classList.add("active")}function Xt(e,t){const o=[];(e==="all"?M.map(a=>a.key):[e]).forEach(a=>{const r=y.filter(c=>c.scores[a]!=null).sort((c,l)=>c.scores[a]-l.scores[a]);for(let c=0;c<r.length-1;c++)for(let l=c+1;l<r.length;l++){const d=Math.abs(r[c].scores[a]-r[l].scores[a]);if(d<=8)o.push({a:r[c],b:r[l],catKey:a,diff:d});else break}}),o.sort((a,r)=>a.diff-r.diff);const s=new Set,n=[];for(const a of o){const r=[a.a.title,a.b.title,a.catKey].join("|");s.has(r)||(s.add(r),n.push(a))}return n.sort(()=>Math.random()-.5).slice(0,t)}function Zt(){const e=Jt[Xe];if(V=Xt(oe,e),V.length===0){alert("Not enough films with close scores to calibrate. Try a different category or add more films.");return}A=0,C={},L={},se=[],y.forEach(t=>{L[t.title]={...t.scores}}),document.getElementById("cal-setup").style.display="none",document.getElementById("cal-matchups").style.display="block",document.getElementById("cal-cat-label").textContent=oe==="all"?"All categories":M.find(t=>t.key===oe)?.label||oe,Be()}function Be(){if(A>=V.length){eo();return}const{a:e,b:t,catKey:o}=V[A],i=V.length,s=Math.round(A/i*100);document.getElementById("cal-progress-label").textContent=`${A+1} / ${i}`,document.getElementById("cal-progress-bar").style.width=s+"%";const n=M.find(c=>c.key===o)?.label||o;L[e.title]?.[o]??e.scores[o],L[t.title]?.[o]??t.scores[o];function a(c,l){const d=c.poster?`<img style="width:100%;height:100%;object-fit:cover;display:block" src="https://image.tmdb.org/t/p/w342${c.poster}" alt="" loading="lazy">`:'<div style="width:100%;height:100%;background:var(--deep-cream)"></div>';return`
