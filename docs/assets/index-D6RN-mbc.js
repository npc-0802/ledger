    ...
    `;return}t.classList.remove("empty"),document.getElementById("mastheadCount").textContent=m.length+" films ranked";const i=document.getElementById("rankings-banner");if(i)if(m.length<10){const s=10-m.length,a=Math.round(m.length/10*100);i.innerHTML=`
        <div style="border-bottom:1px solid var(--rule);padding:12px 0 0;margin-bottom:16px">
          <div style="display:flex;align-items:baseline;justify-content:space-between;margin-bottom:10px">
            <div>
              <span style="font-family:'DM Mono',monospace;font-size:9px;letter-spacing:2px;text-transform:uppercase;color:var(--dim)">Your palate is forming &nbsp;·&nbsp; ${m.length} of 10</span>
              <span style="font-family:'DM Sans',sans-serif;font-size:13px;color:var(--dim);margin-left:14px">Rate ${s} more film${s!==1?"s":""} to unlock Predict and deeper insights.</span>
            </div>
          ...
          <div style="height:2px;background:var(--rule);border-radius:1px">
            <div style="height:2px;width:${a}%;background:var(--blue);border-radius:1px;transition:width 0.4s ease"></div>
          </div>
