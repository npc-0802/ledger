import{C as p,M as y,c as S}from"./index-EyOdYnBo.js";const j="https://ledger-proxy.noahparikhcott.workers.dev",C="palate_insights_v1";function f(){try{return JSON.parse(localStorage.getItem(C)||"{}")}catch{return{}}}function m(e){try{localStorage.setItem(C,JSON.stringify(e))}catch{}}function x(e,t,s){return!e||Math.abs((e.filmCount||0)-t)>=2||e.scoreKey!==s}function I(){const e={};return p.forEach(t=>{const s=y.filter(n=>n.scores[t.key]!=null).map(n=>n.scores[t.key]);e[t.key]=s.length?Math.round(s.reduce((n,c)=>n+c,0)/s.length):null}),e}async function E(e,t){return((await(await fetch(j,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({system:e,messages:[{role:"user",content:t}]})})).json()).content?.[0]?.text||"").trim()}async function O(e,t,s){const n=f(),c=`${e}::${t}`,l=s.length,d=Math.round(s.reduce((a,h)=>a+(h.total||0),0)/l),g=`${d}:${s.map(a=>a.total).sort().join(",")}`;if(!x(n[c],l,g))return n[c].text;const v=I(),w=p.map(a=>`${a.label} ${v[a.key]??"—"}`).join(", "),k=S?.archetype||"unknown",$=[...s].sort((a,h)=>h.total-a.total).map(a=>{const h=p.map(b=>`${b.label.toLowerCase()}=${a.scores[b.key]??"—"}`).join(", ");return`- ${a.title} (${a.year||"?"}): total=${a.total}, ${h}`}).join(`
