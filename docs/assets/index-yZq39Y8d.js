  ...
  `,document.getElementById("filmModal").classList.add("open"),localStorage.setItem("ledger_last_modal",e)}window.modalEnterEdit=function(){const e=y[ue];O=!0,j={...e.scores},ve()};window.modalCancelEdit=function(){O=!1,j={},ve()};window.modalUpdateScore=function(e,t){j[e]=parseInt(t);const o=document.getElementById(`modal-edit-val-${e}`);o&&(o.textContent=t,o.className=`breakdown-val ${W(parseInt(t))}`);const i=document.getElementById(`modal-edit-lbl-${e}`);i&&(i.textContent=we(parseInt(t)));const n=V(j),s=document.getElementById("modal-total-display");s&&(s.textContent=n);const a=document.getElementById("modal-total-label");a&&(a.textContent=we(n))};window.modalSaveScores=function(){const e=y[ue];e.scores={...j},e.total=V(j),O=!1,j={},ae(),Y(),P(),Ie().catch(t=>console.warn("sync failed",t)),ve()};function Tt(e){(!e||e.target===document.getElementById("filmModal"))&&document.getElementById("filmModal").classList.remove("open")}let Z="directors";function R(e){return be((e||"").split(",").map(t=>t.trim()).filter(Boolean))}function At(e){const t={};return y.forEach(o=>{let i=[];e==="directors"?i=R(o.director):e==="writers"?i=R(o.writer):e==="actors"?i=R(o.cast):e==="companies"&&(i=R(o.productionCompanies)),i.forEach(n=>{t[n]||(t[n]=[]),t[n].push(o)})}),t}function Ye(e){const t=At(e);return Object.entries(t).filter(([,o])=>o.length>=2).map(([o,i])=>({name:o,films:i,avg:parseFloat((i.reduce((n,s)=>n+s.total,0)/i.length).toFixed(1)),catAvgs:w.reduce((n,s)=>{const a=i.filter(l=>l.scores[s.key]!=null).map(l=>l.scores[s.key]);return n[s.key]=a.length?parseFloat((a.reduce((l,p)=>l+p,0)/a.length).toFixed(1)):null,n},{})})).sort((o,i)=>i.avg-o.avg)}function Ve(e){return e>=90?"#C4922A":e>=80?"#1F4A2A":e>=70?"#4A5830":e>=60?"#6B4820":"rgba(12,11,9,0.55)"}function ge(e){e&&(Z=e);const t=["directors","writers","actors","companies"],o={directors:"Directors",writers:"Writers",actors:"Actors",companies:"Production Co."},i=Ye(Z);document.getElementById("exploreContent").innerHTML=`
    <div style="max-width:800px">
      ...
      <div class="explore-tabs">
        ${t.map(n=>`<button class="explore-tab ${n===Z?"active":""}" onclick="renderExploreIndex('${n}')">${o[n]}</button>`).join("")}
      </div>

      <div style="height:28px"></div>
      ${i.length===0?`<div style="font-family:'DM Sans',sans-serif;font-size:14px;color:var(--dim);font-style:italic;padding:48px 0">Not enough data yet — add more films to see patterns.</div>`:i.map((n,s)=>{const a=n.name.replace(/'/g,"\\'");return`<div style="display:flex;align-items:center;gap:16px;padding:14px 0;border-bottom:1px solid var(--rule);cursor:pointer" onclick="exploreEntity('${Z==="companies"?"company":Z.slice(0,-1)}','${a}')" onmouseover="this.style.background='var(--cream)'" onmouseout="this.style.background=''">
...
Respond with this exact JSON structure:
{
  ...
  `}window.undoCalChoice=function(){if(ie.length===0)return;const e=ie.pop();C=e.idx,_=e.tempScores,S=e.deltas,Ce()};window.calChoose=function(e){if(ie.push({idx:C,tempScores:JSON.parse(JSON.stringify(_)),deltas:JSON.parse(JSON.stringify(S))}),e!=="skip"){const{a:t,b:o,catKey:i}=H[C],n=_[t.title]?.[i]??t.scores[i],s=_[o.title]?.[i]??o.scores[i],a=1/(1+Math.pow(10,(s-n)/40)),l=1-a,p=e==="a"?1:0,m=1-p,r=Math.round(Math.min(100,Math.max(1,n+Pe*(p-a)))),d=Math.round(Math.min(100,Math.max(1,s+Pe*(m-l))));if(S[t.title]||(S[t.title]={}),S[o.title]||(S[o.title]={}),r!==n){const k=S[t.title][i]?.old??n;S[t.title][i]={old:k,new:r},_[t.title][i]=r}if(d!==s){const k=S[o.title][i]?.old??s;S[o.title][i]={old:k,new:d},_[o.title][i]=d}const g=document.getElementById(`cal-card-${e}`),u=document.getElementById(`cal-card-${e==="a"?"b":"a"}`);g&&(g.style.opacity="1"),u&&(u.style.opacity="0.35",u.style.transform="scale(0.97)")}C++,setTimeout(()=>Ce(),e==="skip"?0:140)};function Jt(){document.getElementById("cal-matchups").style.display="none",document.getElementById("cal-review").style.display="block";const e=Object.entries(S).flatMap(([o,i])=>Object.entries(i).map(([n,{old:s,new:a}])=>({title:o,catKey:n,old:s,new:a}))).filter(o=>o.old!==o.new).sort((o,i)=>Math.abs(i.new-i.old)-Math.abs(o.new-o.old));if(e.length===0){document.getElementById("cal-review-header").innerHTML=`
  ...
  </div>`}function st(e){F[e].checked=!F[e].checked;const t=document.getElementById("companyChip_"+e);t.className="company-chip "+(F[e].checked?"checked":"unchecked")}function at(){ne=null,document.getElementById("tmdb-search-phase").style.display="block",document.getElementById("tmdb-curation-phase").style.display="none",document.getElementById("tmdb-results").innerHTML=""}function rt(){const e=f._allDirectors||[],t=f._allWriters||[],o=Object.values(B).filter(n=>n.checked).map(n=>n.actor.name),i=Object.values(F).filter(n=>n.checked).map(n=>n.company.name);f.title=f._tmdbDetail.title,f.director=e.join(", "),f.writer=t.join(", "),f.cast=o.join(", "),f.productionCompanies=i.join(", "),no(),ce(2)}let ne=null;function oo(e){ne=e}function io(e){const t=[...y].filter(s=>s.scores[e]!=null).sort((s,a)=>a.scores[e]-s.scores[e]),o=t.length,i=[t[Math.floor(o*.05)],t[Math.floor(o*.25)],t[Math.floor(o*.5)],t[Math.floor(o*.75)],t[Math.floor(o*.95)]].filter(Boolean),n=new Set;return i.filter(s=>n.has(s.title)?!1:(n.add(s.title),!0))}function no(){const e=document.getElementById("calibrationCategories");e.innerHTML=w.map(t=>{const o=io(t.key),i=ne?.[t.key]??f.scores[t.key]??50;return`<div class="category-section" id="catSection_${t.key}">
    ...
    </div>`}).join(""),w.forEach(t=>{f.scores[t.key]=ne?.[t.key]??f.scores[t.key]??50})}window.selectAnchor=function(e,t,o){o.closest(".anchor-row").querySelectorAll(".anchor-film").forEach(s=>s.classList.remove("selected")),o.classList.add("selected");const i=f.scores[e]??50,n=Math.round((i+t)/2);document.getElementById("slider_"+e).value=n,updateSlider(e,n)};window.updateSlider=function(e,t){t=parseInt(t),f.scores[e]=t,document.getElementById("sliderVal_"+e).textContent=t,document.getElementById("sliderDesc_"+e).textContent=U(t)};function lt(){so(),ce(3)}let N=[],L=0,se=[];function so(){N=[],se=[],w.forEach(e=>{const t=f.scores[e.key];if(!t)return;y.filter(i=>i.scores[e.key]!=null&&Math.abs(i.scores[e.key]-t)<=3).sort((i,n)=>Math.abs(i.scores[e.key]-t)-Math.abs(n.scores[e.key]-t)).slice(0,1).forEach(i=>N.push({cat:e,film:i}))}),N=N.slice(0,6),L=0,xe()}function xe(){const e=document.getElementById("hthContainer");if(N.length===0||L>=N.length){e.innerHTML=`<div style="text-align:center;padding:40px;color:var(--dim);font-style:italic">
