(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))i(n);new MutationObserver(n=>{for(const a of n)if(a.type==="childList")for(const s of a.addedNodes)s.tagName==="LINK"&&s.rel==="modulepreload"&&i(s)}).observe(document,{childList:!0,subtree:!0});function o(n){const a={};return n.integrity&&(a.integrity=n.integrity),n.referrerPolicy&&(a.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?a.credentials="include":n.crossOrigin==="anonymous"?a.credentials="omit":a.credentials="same-origin",a}function i(n){if(n.ep)return;n.ep=!0;const a=o(n);fetch(n.href,a)}})();const x=[{key:"plot",label:"Plot",weight:3,question:"How strong, original, and well-constructed is the story?"},{key:"execution",label:"Execution",weight:3,question:"Direction, cinematography, pacing — how well is it made?"},{key:"acting",label:"Acting",weight:2,question:"How effective is the overall performance?"},{key:"production",label:"Production",weight:1,question:"Score, production design, costume — the craft around the film."},{key:"enjoyability",label:"Enjoyability",weight:4,question:"The most honest question: how much did you actually enjoy it?"},{key:"rewatchability",label:"Rewatchability",weight:1,question:"Would you sit down and watch this again? How eagerly?"},{key:"ending",label:"Ending",weight:1,question:"How satisfying, earned, and well-executed is the conclusion?"},{key:"uniqueness",label:"Uniqueness",weight:2,question:"Does this feel genuinely singular? Could only this film exist this way?"}];let u=[],g=null;function Z(e){g=e}function ee(e){u.length=0,e.forEach(t=>u.push(t))}const Qe=[[90,"An all-time favorite"],[85,"Really quite exceptional"],[80,"Excellent"],[75,"Well above average"],[70,"Great"],[65,"Very good"],[60,"A cut above"],[55,"Good"],[50,"Solid"],[45,"Not bad"],[40,"Sub-par"],[35,"Multiple flaws"],[30,"Poor"],[25,"Bad"],[20,"Wouldn't watch by choice"],[0,"Unwatchable"]];function C(e){if(e>=90&&e===Math.max(...u.map(t=>t.total)))return"No better exists";for(const[t,o]of Qe)if(e>=t)return o;return"Unwatchable"}function te(e){let t=0,o=0;for(const i of x)e[i.key]!=null&&(t+=e[i.key]*i.weight,o+=i.weight);return o>0?Math.round(t/o*100)/100:0}function oe(){u.forEach(e=>{e.total=te(e.scores)})}function z(e){return e>=90?"s90":e>=80?"s80":e>=70?"s70":e>=60?"s60":e>=50?"s50":e>=40?"s40":"s30"}function V(){if(!g||!g.weights)return;const e=g.weights;x.forEach(t=>{e[t.key]!=null&&(t.weight=e[t.key])}),oe()}let j={key:"rank",dir:"desc"};function be(e){j.key===e?j.dir=j.dir==="desc"?"asc":"desc":(j.key=e,j.dir="desc"),document.querySelectorAll(".sort-arrow").forEach(o=>o.classList.remove("active-sort"));const t=document.getElementById("sort-"+e+"-arrow")||document.getElementById("sort-"+e);if(t){const o=t.querySelector?t.querySelector(".sort-arrow"):t;o&&(o.classList.add("active-sort"),o.textContent=j.dir==="desc"?"↓":"↑")}A()}function A(){const e=[...u].sort((s,l)=>l.total-s.total),t=new Map(e.map((s,l)=>[s.title,l+1]));let o;const{key:i,dir:n}=j;i==="rank"||i==="total"?o=[...u].sort((s,l)=>n==="desc"?l.total-s.total:s.total-l.total):i==="title"?o=[...u].sort((s,l)=>n==="desc"?l.title.localeCompare(s.title):s.title.localeCompare(l.title)):o=[...u].sort((s,l)=>n==="desc"?(l.scores[i]||0)-(s.scores[i]||0):(s.scores[i]||0)-(l.scores[i]||0)),document.getElementById("mastheadCount").textContent=o.length+" films ranked";const a=document.getElementById("filmList");a.innerHTML=o.map(s=>{const l=s.scores,r=t.get(s.title);return`<div class="film-row" onclick="openModal(${u.indexOf(s)})">
      ...
      ${["plot","execution","acting","production","enjoyability","rewatchability","ending","uniqueness"].map(c=>`<div class="film-score ${l[c]?z(l[c]):""}">${l[c]??"—"}</div>`).join("")}
      <div class="film-total">${s.total}</div>
    </div>`}).join("")}const Xe=Object.freeze(Object.defineProperty({__proto__:null,renderRankings:A,sortBy:be},Symbol.toStringTag,{value:"Module"}));function Ze(e){const t=u[e],o=[...u].sort((d,v)=>v.total-d.total),i=o.indexOf(t)+1,n=o.filter(d=>d!==t&&Math.abs(d.total-t.total)<6).slice(0,5),a={};x.forEach(d=>{const v=[...u].sort((f,E)=>(E.scores[d.key]||0)-(f.scores[d.key]||0));a[d.key]=v.indexOf(t)+1});const s=(d,v,f)=>`<span class="modal-meta-chip" onclick="exploreEntity('${v}','${f.replace(/'/g,"'")}')">${d}</span>`,l=(t.director||"").split(",").map(d=>d.trim()).filter(Boolean).map(d=>s(d,"director",d)).join(""),r=(t.writer||"").split(",").map(d=>d.trim()).filter(Boolean).filter(d=>!(t.director||"").includes(d)).map(d=>s(d,"writer",d)).join(""),c=(t.cast||"").split(",").map(d=>d.trim()).filter(Boolean).map(d=>s(d,"actor",d)).join(""),p=t.poster?`<img class="modal-poster" src="https://image.tmdb.org/t/p/w780${t.poster}" alt="${t.title}">`:`<div class="modal-poster-placeholder">${t.title} · ${t.year||""}</div>`;document.getElementById("modalContent").innerHTML=`
    ${p}
    ...
    <div style="display:flex;align-items:baseline;gap:12px;margin-bottom:20px">
      ...
      <span style="font-family:'DM Mono',monospace;font-size:12px;color:var(--dim)">${C(t.total)}</span>
    </div>
    <div>${x.map(d=>{const v=t.scores[d.key],f=a[d.key];return`<div class="breakdown-row">
        <div class="breakdown-cat">${d.label}</div>
        <div class="breakdown-bar-wrap"><div class="breakdown-bar" style="width:${v||0}%"></div></div>
        <div class="breakdown-val ${v?z(v):""}">${v??"—"}</div>
    ...
    ${n.length>0?`<div class="compare-section">
      ...
      ${n.map(d=>{const v=(d.total-t.total).toFixed(2),f=v>0?"+":"";return`<div class="compare-film" style="cursor:pointer" onclick="closeModal();openModal(${u.indexOf(d)})">
          ...
          <div class="compare-diff ${v>0?"diff-pos":"diff-neg"}">${f}${v}</div>
        </div>`}).join("")}
    </div>`:""}
  `,document.getElementById("filmModal").classList.add("open"),localStorage.setItem("ledger_last_modal",e)}function et(e){(!e||e.target===document.getElementById("filmModal"))&&document.getElementById("filmModal").classList.remove("open")}let K="directors";function tt(e){const t={};return u.forEach(o=>{let i=[];e==="directors"?i=(o.director||"").split(",").map(n=>n.trim()).filter(Boolean):e==="writers"?i=(o.writer||"").split(",").map(n=>n.trim()).filter(Boolean):e==="actors"?i=(o.cast||"").split(",").map(n=>n.trim()).filter(Boolean):e==="companies"&&(i=(o.productionCompanies||"").split(",").map(n=>n.trim()).filter(Boolean)),i.forEach(n=>{t[n]||(t[n]=[]),t[n].push(o)})}),t}function xe(e){const t=tt(e);return Object.entries(t).filter(([,o])=>o.length>=2).map(([o,i])=>({name:o,films:i,avg:parseFloat((i.reduce((n,a)=>n+a.total,0)/i.length).toFixed(1)),catAvgs:x.reduce((n,a)=>{const s=i.filter(l=>l.scores[a.key]!=null).map(l=>l.scores[a.key]);return n[a.key]=s.length?parseFloat((s.reduce((l,r)=>l+r,0)/s.length).toFixed(1)):null,n},{})})).sort((o,i)=>i.avg-o.avg)}function ie(e){e&&(K=e);const t=["directors","writers","actors"],o={directors:"Directors",writers:"Writers",actors:"Actors"},i=xe(K);document.getElementById("exploreContent").innerHTML=`
    <div style="max-width:960px">
      ...
      ${i.length===0?'<div style="color:var(--dim);font-style:italic;padding:40px 0">Not enough data yet — add more films to see patterns.</div>':`<div class="explore-index">
          ${i.map((n,a)=>`
            <div class="explore-index-card" onclick="exploreEntity('${K.slice(0,-1)}','${n.name.replace(/'/g,"\\'")}')">
  ...
  `}function ot(e,t){document.getElementById("filmModal").classList.remove("open"),document.querySelectorAll(".screen").forEach(y=>y.classList.remove("active")),document.getElementById("explore").classList.add("active"),document.querySelectorAll(".nav-btn").forEach(y=>y.classList.remove("active")),document.querySelectorAll(".nav-btn")[1].classList.add("active"),window.scrollTo(0,0),document.getElementById("exploreContent").scrollTop=0;const o=e==="director"?"directors":e==="writer"?"writers":"actors",i=e==="director"?"Director":e==="writer"?"Writer":"Actor",n=u.filter(y=>e==="director"?(y.director||"").split(",").map(b=>b.trim()).includes(t):e==="writer"?(y.writer||"").split(",").map(b=>b.trim()).includes(t):e==="actor"?(y.cast||"").split(",").map(b=>b.trim()).includes(t):!1).sort((y,b)=>b.total-y.total);if(n.length===0){ie();return}const a=xe(o),s=a.findIndex(y=>y.name===t)+1,l=a.length,r=a.find(y=>y.name===t),c=r?r.avg.toFixed(1):(n.reduce((y,b)=>y+b.total,0)/n.length).toFixed(1),p=n[0],d={};x.forEach(y=>{const b=a.filter(L=>L.catAvgs[y.key]!=null).sort((L,Ke)=>Ke.catAvgs[y.key]-L.catAvgs[y.key]),w=b.findIndex(L=>L.name===t)+1;d[y.key]=w>0?{rank:w,total:b.length}:null});const f=x.map(y=>{const b=n.filter(w=>w.scores[y.key]!=null).map(w=>w.scores[y.key]);return{...y,avg:b.length?parseFloat((b.reduce((w,L)=>w+L,0)/b.length).toFixed(1)):null}}).filter(y=>y.avg!=null).sort((y,b)=>b.avg-y.avg),E=f[0],I=f[f.length-1];document.getElementById("exploreContent").innerHTML=`
    <div style="max-width:960px">
      ...
      <div class="explore-stat-row">
        ...
        <div class="explore-stat">
          <div class="explore-stat-val ${z(p.total)}">${p.total}</div>
      ...
      ${f.length>0?`
        ...
        <div class="explore-cat-breakdown">
          ${f.map(y=>{const b=d[y.key];return`
            <div class="explore-cat-cell">
              <div class="explore-cat-cell-label">${y.label}</div>
              <div class="explore-cat-cell-val ${z(y.avg)}">${y.avg.toFixed(1)}</div>
              ${b?`<div style="font-family:'DM Mono',monospace;font-size:9px;color:var(--dim);margin-top:2px">#${b.rank} of ${b.total}</div>`:""}
            </div>`}).join("")}
        </div>
        ...
        ${E&&I&&E.key!==I.key?`
          <div style="background:var(--blue-pale);border:1px solid var(--rule);padding:16px 20px;margin:20px 0;font-size:13px;line-height:1.7;color:var(--ink)">
            You rate ${t}'s <strong>${E.label.toLowerCase()}</strong> highest (avg ${E.avg.toFixed(1)})${I.avg<70?`, but find their <strong>${I.label.toLowerCase()}</strong> less compelling (avg ${I.avg.toFixed(1)})`:""}.
      ...
      ${n.map((y,b)=>`
        <div class="film-row" onclick="openModal(${u.indexOf(y)})" style="cursor:pointer">
          <div class="film-rank">${b+1}</div>
          <div class="film-title-cell">
            <div class="film-title-main">${y.title}</div>
            <div class="film-title-sub">${y.year||""} · ${y.director||""}</div>
          </div>
          ${["plot","execution","acting","production","enjoyability","rewatchability","ending","uniqueness"].map(w=>`<div class="film-score ${y.scores[w]?z(y.scores[w]):"}"}">${y.scores[w]??"—"}</div>`).join("")}
          <div class="film-total">${y.total}</div>
        </div>`).join("")}
    </div>
  `}function we(){const e={},t={},o={};u.forEach(r=>{r.director.split(",").forEach(c=>{c=c.trim(),c&&(e[c]||(e[c]=[]),e[c].push(r.total))}),r.cast.split(",").forEach(c=>{c=c.trim(),c&&(t[c]||(t[c]=[]),t[c].push(r.total))}),r.year&&(o[r.year]||(o[r.year]=[]),o[r.year].push(r.total))});const i=r=>Math.round(r.reduce((c,p)=>c+p,0)/r.length*100)/100,n=Object.entries(e).filter(([,r])=>r.length>=2).map(([r,c])=>({name:r,avg:i(c),count:c.length})).sort((r,c)=>c.avg-r.avg).slice(0,10),a=Object.entries(t).filter(([,r])=>r.length>=2).map(([r,c])=>({name:r,avg:i(c),count:c.length})).sort((r,c)=>c.avg-r.avg).slice(0,10),s=Object.entries(o).filter(([,r])=>r.length>=2).map(([r,c])=>({name:r,avg:i(c),count:c.length})).sort((r,c)=>c.avg-r.avg).slice(0,10),l=x.map(r=>{const c=u.map(p=>p.scores[r.key]).filter(p=>p!=null);return{...r,avg:i(c)}});document.getElementById("analysisContent").innerHTML=`
  ...
  `}const it="modulepreload",nt=function(e){return"/ledger/"+e},fe={},M=function(t,o,i){let n=Promise.resolve();if(o&&o.length>0){let c=function(p){return Promise.all(p.map(d=>Promise.resolve(d).then(v=>({status:"fulfilled",value:v}),v=>({status:"rejected",reason:v}))))};var s=c;document.getElementsByTagName("link");const l=document.querySelector("meta[property=csp-nonce]"),r=l?.nonce||l?.getAttribute("nonce");n=c(o.map(p=>{if(p=nt(p),p in fe)return;fe[p]=!0;const d=p.endsWith(".css"),v=d?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${p}"]${v}`))return;const f=document.createElement("link");if(f.rel=d?"stylesheet":it,d||(f.as="script"),f.crossOrigin="",f.href=p,r&&f.setAttribute("nonce",r),document.head.appendChild(f),d)return new Promise((E,I)=>{f.addEventListener("load",E),f.addEventListener("error",()=>I(new Error(`Unable to preload CSS for ${p}`)))})}))}function a(l){const r=new Event("vite:preloadError",{cancelable:!0});if(r.payload=l,window.dispatchEvent(r),!r.defaultPrevented)throw l}return n.then(l=>{for(const r of l||[])r.status==="rejected"&&a(r.reason);return t().catch(a)})},ae="f5a446a5f70a9f6a16a8ddd052c121f2",re="https://api.themoviedb.org/3",st="https://ledger-proxy.noahparikhcott.workers.dev";let ve=null,O=null;function $e(){document.getElementById("predict-search").value="",document.getElementById("predict-search-results").innerHTML="",document.getElementById("predict-result").innerHTML="",O=null,setTimeout(()=>document.getElementById("predict-search")?.focus(),50)}function at(){clearTimeout(ve),ve=setTimeout(ke,500)}async function ke(){const e=document.getElementById("predict-search").value.trim();if(!e||e.length<2)return;const t=document.getElementById("predict-search-results");t.innerHTML='<div class="tmdb-loading">Searching…</div>';try{const n=((await(await fetch(`${re}/search/movie?api_key=${ae}&query=${encodeURIComponent(e)}&language=en-US&page=1`)).json()).results||[]).slice(0,5);if(!n.length){t.innerHTML='<div class="tmdb-error">No results found.</div>';return}const a=new Set(u.map(s=>s.title.toLowerCase()));t.innerHTML=n.map(s=>{const l=s.release_date?.slice(0,4)||"",r=s.poster_path?`<img class="tmdb-result-poster" src="https://image.tmdb.org/t/p/w92${s.poster_path}">`:'<div class="tmdb-result-poster-placeholder">no img</div>',c=a.has(s.title.toLowerCase());return`<div class="tmdb-result ${c?"opacity-50":""}" onclick="${c?"":`predictSelectFilm(${s.id}, '${s.title.replace(/'/g,"\\'")}', '${l}')`}" style="${c?"opacity:0.4;cursor:default":""}">
        ${r}
    ...
    </div>`;let i={},n={};try{const[d,v]=await Promise.all([fetch(`${re}/movie/${e}?api_key=${ae}`),fetch(`${re}/movie/${e}/credits?api_key=${ae}`)]);i=await d.json(),n=await v.json()}catch{}const a=(n.crew||[]).filter(d=>d.job==="Director").map(d=>d.name).join(", "),s=(n.crew||[]).filter(d=>["Screenplay","Writer","Story"].includes(d.job)).map(d=>d.name).slice(0,2).join(", "),l=(n.cast||[]).slice(0,8).map(d=>d.name).join(", "),r=(i.genres||[]).map(d=>d.name).join(", "),c=i.overview||"",p=i.poster_path||null;O={tmdbId:e,title:t,year:o,director:a,writer:s,cast:l,genres:r,overview:c,poster:p},await dt(O)}function lt(){const e=["plot","execution","acting","production","enjoyability","rewatchability","ending","uniqueness"],t={};e.forEach(s=>{const l=u.filter(p=>p.scores[s]!=null).map(p=>p.scores[s]);if(!l.length){t[s]={mean:70,std:10,min:0,max:100};return}const r=l.reduce((p,d)=>p+d,0)/l.length,c=Math.sqrt(l.reduce((p,d)=>p+(d-r)**2,0)/l.length);t[s]={mean:Math.round(r*10)/10,std:Math.round(c*10)/10,min:Math.min(...l),max:Math.max(...l)}});const o=[...u].sort((s,l)=>l.total-s.total),i=o.slice(0,10).map(s=>`${s.title} (${s.total})`).join(", "),n=o.slice(-5).map(s=>`${s.title} (${s.total})`).join(", "),a=x.map(s=>`${s.label}×${s.weight}`).join(", ");return{stats:t,top10:i,bottom5:n,weightStr:a,archetype:g?.archetype,archetypeSecondary:g?.archetype_secondary,totalFilms:u.length}}function ct(e){const t=(e.director||"").split(",").map(i=>i.trim()).filter(Boolean),o=(e.cast||"").split(",").map(i=>i.trim()).filter(Boolean);return u.filter(i=>{const n=(i.director||"").split(",").map(s=>s.trim()),a=(i.cast||"").split(",").map(s=>s.trim());return t.some(s=>n.includes(s))||o.some(s=>a.includes(s))}).sort((i,n)=>n.total-i.total).slice(0,8)}async function dt(e){const t=lt(),o=ct(e),i=o.length?o.map(l=>`- ${l.title} (${l.year||""}): total=${l.total}, plot=${l.scores.plot}, execution=${l.scores.execution}, acting=${l.scores.acting}, production=${l.scores.production}, enjoyability=${l.scores.enjoyability}, rewatchability=${l.scores.rewatchability}, ending=${l.scores.ending}, uniqueness=${l.scores.uniqueness}`).join(`
...
Respond with this exact JSON structure:
{
  ...
  "key_comparables": ["<film title>", "<film title>"]
}`;try{const p=((await(await fetch(st,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({system:a,messages:[{role:"user",content:s}]})})).json()).content?.[0]?.text||"").replace(/```json|```/g,"").trim(),d=JSON.parse(p);pt(e,d,o)}catch(l){document.getElementById("predict-result").innerHTML=`
      <div class="tmdb-error">Prediction failed: ${l.message}. Check that the proxy is running and your API key is valid.</div>`}}function pt(e,t,o){let i=0,n=0;x.forEach(c=>{const p=t.predicted_scores[c.key];p!=null&&(i+=p*c.weight,n+=c.weight)});const a=n>0?Math.round(i/n*100)/100:0,s=e.poster?`<img class="predict-poster" src="https://image.tmdb.org/t/p/w185${e.poster}" alt="${e.title}">`:`<div class="predict-poster-placeholder">${e.title}</div>`,l={high:"conf-high",medium:"conf-medium",low:"conf-low"}[t.confidence]||"conf-medium",r={high:"High confidence",medium:"Medium confidence",low:"Low confidence"}[t.confidence]||"";document.getElementById("predict-result").innerHTML=`
    ...
    <div class="predict-film-card">
      ...
      <div style="flex:1">
        ...
        <div style="display:flex;align-items:baseline;gap:8px">
          ...
          <div>
            <div style="font-family:'DM Mono',monospace;font-size:12px;color:var(--dim)">${C(a)}</div>
    ...
    <div class="predict-score-grid">
      ${x.map(c=>{const p=t.predicted_scores[c.key];return`<div class="predict-score-cell">
          ...
          <div class="predict-score-cell-val ${p?z(p):""}">${p??"—"}</div>
        </div>`}).join("")}
    </div>
  ...
  `}function mt(){O&&(document.querySelectorAll(".screen").forEach(e=>e.classList.remove("active")),document.getElementById("add").classList.add("active"),document.querySelectorAll(".nav-btn").forEach(e=>e.classList.remove("active")),document.querySelector('.nav-btn[onclick*="add"]').classList.add("active"),setTimeout(()=>{const e=document.getElementById("f-search");e&&(e.value=O.title,M(()=>Promise.resolve().then(()=>Dt),void 0).then(t=>t.liveSearch(O.title)))},100))}const Me="filmRankings_v1";function H(){try{localStorage.setItem(Me,JSON.stringify(u))}catch(e){console.warn("localStorage save failed:",e)}g&&(clearTimeout(H._syncTimer),H._syncTimer=setTimeout(()=>{M(()=>Promise.resolve().then(()=>Be),void 0).then(e=>e.syncToSupabase())},2e3))}function ut(){try{const e=localStorage.getItem(Me);if(!e)return;const t=JSON.parse(e);if(!Array.isArray(t)||t.length===0)return;ee(t),console.log(`Loaded ${u.length} films from localStorage`)}catch(e){console.warn("localStorage load failed:",e)}}let W="all",Ee="focused",P=[],T=0,$={},_={};const yt={focused:15,thorough:30,deep:50},he=8;function ft(e){W=e,document.querySelectorAll('[id^="calcat_"]').forEach(t=>t.className="company-chip"),document.getElementById("calcat_"+e).className="company-chip checked"}function vt(e){Ee=e,document.querySelectorAll('[id^="calint_"]').forEach(t=>t.className="company-chip"),document.getElementById("calint_"+e).className="company-chip checked"}function ht(e,t){const o=[];(e==="all"?x.map(s=>s.key):[e]).forEach(s=>{const l=u.filter(r=>r.scores[s]!=null).sort((r,c)=>r.scores[s]-c.scores[s]);for(let r=0;r<l.length-1;r++)for(let c=r+1;c<l.length;c++){const p=Math.abs(l[r].scores[s]-l[c].scores[s]);if(p<=8)o.push({a:l[r],b:l[c],catKey:s,diff:p});else break}}),o.sort((s,l)=>s.diff-l.diff);const n=new Set,a=[];for(const s of o){const l=[s.a.title,s.b.title,s.catKey].join("|");n.has(l)||(n.add(l),a.push(s))}return a.sort(()=>Math.random()-.5).slice(0,t)}function gt(){const e=yt[Ee];if(P=ht(W,e),P.length===0){alert("Not enough films with close scores to calibrate. Try a different category or add more films.");return}T=0,$={},_={},u.forEach(t=>{_[t.title]={...t.scores}}),document.getElementById("cal-setup").style.display="none",document.getElementById("cal-matchups").style.display="block",document.getElementById("cal-cat-label").textContent=W==="all"?"All categories":x.find(t=>t.key===W)?.label||W,Se()}function Se(){if(T>=P.length){bt();return}const{a:e,b:t,catKey:o}=P[T],i=P.length,n=Math.round(T/i*100);document.getElementById("cal-progress-label").textContent=`Matchup ${T+1} of ${i}`,document.getElementById("cal-progress-bar").style.width=n+"%";const a=x.find(r=>r.key===o)?.label||o,s=_[e.title]?.[o]??e.scores[o],l=_[t.title]?.[o]??t.scores[o];document.getElementById("cal-matchup-card").innerHTML=`
    ...
    <div class="hth-row">
      <div class="hth-card" onclick="calChoose('a')">
        ...
        <div style="font-family:'DM Mono',monospace;font-size:10px;color:var(--dim);margin-top:4px">${C(s)}</div>
      </div>
      ...
      <div class="hth-card" onclick="calChoose('b')">
        ...
        <div style="font-family:'DM Mono',monospace;font-size:10px;color:var(--dim);margin-top:4px">${C(l)}</div>
      </div>
    </div>
    <div class="hth-skip" onclick="calChoose('skip')">Too close to call — skip</div>
  `}window.calChoose=function(e){if(e!=="skip"){const{a:t,b:o,catKey:i}=P[T],n=_[t.title]?.[i]??t.scores[i],a=_[o.title]?.[i]??o.scores[i],s=1/(1+Math.pow(10,(a-n)/40)),l=1-s,r=e==="a"?1:0,c=1-r,p=Math.round(Math.min(100,Math.max(1,n+he*(r-s)))),d=Math.round(Math.min(100,Math.max(1,a+he*(c-l))));if($[t.title]||($[t.title]={}),$[o.title]||($[o.title]={}),p!==n){const v=$[t.title][i]?.old??n;$[t.title][i]={old:v,new:p},_[t.title][i]=p}if(d!==a){const v=$[o.title][i]?.old??a;$[o.title][i]={old:v,new:d},_[o.title][i]=d}}T++,Se()};function bt(){document.getElementById("cal-matchups").style.display="none",document.getElementById("cal-review").style.display="block";const e=Object.entries($).flatMap(([o,i])=>Object.entries(i).map(([n,{old:a,new:s}])=>({title:o,catKey:n,old:a,new:s}))).filter(o=>o.old!==o.new).sort((o,i)=>Math.abs(i.new-i.old)-Math.abs(o.new-o.old));if(e.length===0){document.getElementById("cal-diff-list").innerHTML=`
  ...
  `}function xt(){try{const e=document.querySelectorAll('[id^="caldiff_"]');let t=0;e.forEach(o=>{if(!o.checked)return;const i=parseInt(o.dataset.movieIdx),n=o.dataset.cat,a=parseInt(o.dataset.new),s=u[i];s&&s.scores[n]!==void 0&&(s.scores[n]=a,s.total=te(s.scores),t++)}),oe(),H(),M(()=>Promise.resolve().then(()=>F),void 0).then(o=>o.updateStorageStatus()),A(),document.querySelectorAll(".screen").forEach(o=>o.classList.remove("active")),document.getElementById("rankings").classList.add("active"),document.querySelectorAll(".nav-btn").forEach(o=>o.classList.remove("active")),document.querySelector('.nav-btn[onclick*="rankings"]').classList.add("active"),de(),alert(`Applied ${t} score change${t!==1?"s":""}. Rankings updated.`)}catch(e){console.error("applyCalibration error:",e),alert("Error applying changes: "+e.message)}}function de(){P=[],T=0,$={},_={},document.getElementById("cal-setup").style.display="block",document.getElementById("cal-matchups").style.display="none",document.getElementById("cal-review").style.display="none",document.getElementById("cal-apply-btn").style.display=""}const ne={Visceralist:{weights:{plot:2,execution:2,acting:2,production:1,enjoyability:5,rewatchability:3,ending:1,uniqueness:1},quote:`"If I'm not feeling it, nothing else matters."`},Formalist:{weights:{plot:2,execution:4,acting:1,production:3,enjoyability:1,rewatchability:1,ending:1,uniqueness:3},quote:'"How you say it matters as much as what you say."'},Narrativist:{weights:{plot:4,execution:2,acting:2,production:1,enjoyability:1,rewatchability:1,ending:3,uniqueness:1},quote:'"A great story can survive almost anything."'},Humanist:{weights:{plot:2,execution:2,acting:4,production:1,enjoyability:3,rewatchability:1,ending:1,uniqueness:1},quote:'"I come for the story, I stay for the people."'},Completionist:{weights:{plot:2,execution:3,acting:1,production:1,enjoyability:1,rewatchability:1,ending:1,uniqueness:4},quote:`"I want something I've never seen before."`},Sensualist:{weights:{plot:1,execution:4,acting:1,production:4,enjoyability:1,rewatchability:1,ending:1,uniqueness:2},quote:'"Cinema is first an aesthetic experience."'},Revisionist:{weights:{plot:1,execution:2,acting:1,production:1,enjoyability:1,rewatchability:4,ending:2,uniqueness:3},quote:'"My first watch is just the beginning."'},Absolutist:{weights:{plot:3,execution:2,acting:1,production:1,enjoyability:1,rewatchability:1,ending:4,uniqueness:2},quote:'"The ending is the argument."'},Atmospherist:{weights:{plot:1,execution:2,acting:1,production:2,enjoyability:3,rewatchability:5,ending:1,uniqueness:1},quote:'"The right film at the right moment is everything."'}},wt=[{q:"You finish a film that you admired more than you enjoyed. How do you rate it?",options:[{key:"A",text:"Rate it highly. The craft speaks for itself."},{key:"B",text:"Rate it somewhere in the middle. Both things are true."},{key:"C",text:"Rate it lower. If it didn't connect, something didn't work."},{key:"D",text:"Watch it again before deciding."}]},{q:"A film you've been completely absorbed in for two hours ends in a way that doesn't satisfy you. How much does that affect how you feel about the whole thing?",options:[{key:"A",text:"A lot. The ending is the argument. It reframes everything before it."},{key:"B",text:"Somewhat. It takes the edge off, but two great hours are still two great hours."},{key:"C",text:"Not much. I was there for the ride, not the destination."},{key:"D",text:"Depends on the film. Some endings are meant to be unresolved."}]},{q:"Think about a film you've seen multiple times. Is there a version of that experience — a specific night, a specific mood, a specific person you watched it with — that you remember more than the film itself?",options:[{key:"A",text:"Yes, and honestly that's a big part of why I love it."},{key:"B",text:"Maybe, but I try to rate the film on its own terms."},{key:"C",text:"Not really. A great film is great regardless of when you watch it."},{key:"D",text:"I don't rewatch much. I'd rather see something new."}]},{q:"It's a Sunday. You have the whole afternoon. You're scrolling through options and you see a film you've seen probably four or five times already. Do you put it on?",options:[{key:"A",text:"Honestly, yeah. Sometimes that's exactly what the moment calls for."},{key:"B",text:"Only if I'm in a specific mood for it. Otherwise I'd rather find something new."},{key:"C",text:"Probably not. There's too much I haven't seen."},{key:"D",text:"Depends who I'm watching with."}]},{q:"Sometimes a performance makes you forget you're watching a film. You're not thinking about the script or the direction — you're just completely inside another person. How much does that experience shape how you feel about a film overall?",options:[{key:"A",text:"It's everything. A performance like that can carry a film for me."},{key:"B",text:"It elevates it, but I need the rest of the film to hold up too."},{key:"C",text:"I notice it, but it's one piece of a bigger picture."},{key:"D",text:"Honestly I'm usually more absorbed by the world the film creates than the people in it."}]},{q:"A film has one of the greatest performances you've ever seen. The script around it is a mess. Where do you land?",options:[{key:"A",text:"Still a great film. That performance is the film."},{key:"B",text:"Good but frustrating. What could have been."},{key:"C",text:"The script drags it down significantly. A film is only as strong as its weakest part."},{key:"D",text:"Depends how bad the script is. There's a threshold."}]}],$t="https://gzuuhjjedrzeqbgxhfip.supabase.co",kt="sb_publishable_OprjtxkrwknRf8jSZ7bYWg_GGqRiu4z",se=window.supabase.createClient($t,kt);async function Ie(){const e=g;if(!e)return;const{setCloudStatus:t}=await M(async()=>{const{setCloudStatus:o}=await Promise.resolve().then(()=>F);return{setCloudStatus:o}},void 0);t("syncing");try{const{error:o}=await se.from("ledger_users").upsert({id:e.id,username:e.username,display_name:e.display_name,archetype:e.archetype,archetype_secondary:e.archetype_secondary,weights:e.weights,harmony_sensitivity:e.harmony_sensitivity||.3,movies:u,updated_at:new Date().toISOString()},{onConflict:"id"});if(o)throw o;t("synced"),J()}catch(o){console.warn("Supabase sync error:",JSON.stringify(o)),t("error")}}async function _e(e){const{setCloudStatus:t,updateMastheadProfile:o,updateStorageStatus:i}=await M(async()=>{const{setCloudStatus:a,updateMastheadProfile:s,updateStorageStatus:l}=await Promise.resolve().then(()=>F);return{setCloudStatus:a,updateMastheadProfile:s,updateStorageStatus:l}},void 0),{renderRankings:n}=await M(async()=>{const{renderRankings:a}=await Promise.resolve().then(()=>Xe);return{renderRankings:a}},void 0);t("syncing");try{const{data:a,error:s}=await se.from("ledger_users").select("*").eq("id",e).single();if(s)throw s;a&&(Z({id:a.id,username:a.username,display_name:a.display_name,archetype:a.archetype,archetype_secondary:a.archetype_secondary,weights:a.weights,harmony_sensitivity:a.harmony_sensitivity}),a.movies&&Array.isArray(a.movies)&&a.movies.length>=u.length&&ee(a.movies),J(),V(),t("synced"),o(),n(),i())}catch(a){console.warn("Supabase load error:",a),t("error")}}function J(){try{localStorage.setItem("ledger_user",JSON.stringify(g))}catch{}}function Ce(){try{const e=localStorage.getItem("ledger_user");e&&Z(JSON.parse(e))}catch{}}const Be=Object.freeze(Object.defineProperty({__proto__:null,loadFromSupabase:_e,loadUserLocally:Ce,saveUserLocally:J,sb:se,syncToSupabase:Ie},Symbol.toStringTag,{value:"Module"}));let h="name",R={},Q="",S=null,X=null;function pe(){const e=document.getElementById("onboarding-overlay");e.style.display="flex",h="name",R={},B()}function B(){const e=document.getElementById("ob-card-content");if(h==="name")e.innerHTML=`
    ...
    `;else if(typeof h=="number"){const t=wt[h],o=Math.round(h/6*100);e.innerHTML=`
      ...
      ${t.options.map(i=>`
        <div class="ob-option ${R[h]===i.key?"selected":""}" onclick="obSelectAnswer(${h}, '${i.key}', this)">
      ...
      <div class="ob-nav">
        ...
        <button class="ob-btn-primary" id="ob-next-btn" onclick="obNext()" ${R[h]?"":"disabled"}>
          ${h===5?"See my archetype →":"Next →"}
        </button>
      </div>
    `}else if(h==="reveal"){const t=Mt(R);S=t,S._slug||(S._slug=Q.toLowerCase().replace(/[^a-z0-9]/g,"-")+"-"+Math.floor(Math.random()*9e3+1e3));const o=ne[t.primary];e.innerHTML=`
    ...
    `,setTimeout(()=>{const i=document.getElementById("ob-reveal-username");i&&(i.textContent=S._slug)},0)}}window.obCheckName=function(){const e=document.getElementById("ob-name-field")?.value?.trim(),t=document.getElementById("ob-name-btn");t&&(t.disabled=!e||e.length<1)};window.obSubmitName=function(){const e=document.getElementById("ob-name-field")?.value?.trim();e&&(Q=e,h=0,B())};window.obShowReturning=function(){h="returning",B()};window.obShowImport=function(){h="import",X=null,B()};window.obHandleImportDrop=function(e){e.preventDefault(),document.getElementById("ob-import-drop").style.borderColor="var(--rule-dark)";const t=e.dataTransfer.files[0];t&&Te(t)};window.obHandleImportFile=function(e){const t=e.files[0];t&&Te(t)};function Te(e){const t=new FileReader;t.onload=o=>{try{const i=JSON.parse(o.target.result);if(!Array.isArray(i)||i.length===0)throw new Error("invalid");if(!i[0].scores||!i[0].title)throw new Error("invalid");X=i,document.getElementById("ob-import-status").textContent=`✓ ${i.length} films ready to import`,document.getElementById("ob-import-status").style.color="var(--green)",document.getElementById("ob-import-drop").style.borderColor="var(--green)",document.getElementById("ob-import-drop").innerHTML=`<div style="font-family:'DM Mono',monospace;font-size:11px;color:var(--green)">${e.name}</div>`,document.getElementById("ob-import-btn").disabled=!1}catch{document.getElementById("ob-import-status").textContent="That doesn't look like a valid ledger JSON file.",document.getElementById("ob-import-status").style.color="var(--red)"}},t.readAsText(e)}window.obConfirmImport=function(){X&&(ee(X),h=0,B())};window.obLookupUser=async function(){const e=document.getElementById("ob-returning-btn"),t=document.getElementById("ob-returning-error"),o=document.getElementById("ob-returning-field")?.value?.trim().toLowerCase();if(o){e.disabled=!0,e.textContent="Looking up…",t.style.display="none";try{const{data:i,error:n}=await se.from("ledger_users").select("*").eq("username",o).single();if(n||!i)throw new Error("not found");Z({id:i.id,username:i.username,display_name:i.display_name,archetype:i.archetype,archetype_secondary:i.archetype_secondary,weights:i.weights,harmony_sensitivity:i.harmony_sensitivity}),i.movies&&Array.isArray(i.movies)&&i.movies.length>0&&ee(i.movies),J(),H(),V(),oe(),document.getElementById("onboarding-overlay").style.display="none";const a=await M(()=>Promise.resolve().then(()=>F),void 0);a.updateMastheadProfile(),a.setCloudStatus("synced"),a.updateStorageStatus(),A()}catch{e.disabled=!1,e.textContent="Restore profile →",t.style.display="block"}}};window.obSelectAnswer=function(e,t,o){R[e]=t,o.closest(".ob-card").querySelectorAll(".ob-option").forEach(n=>n.classList.remove("selected")),o.classList.add("selected");const i=document.getElementById("ob-next-btn");i&&(i.disabled=!1)};window.obBack=function(){h>0?(h--,B()):(h="name",B())};window.obNext=function(){R[h]&&(h<5?(h++,B()):(h="reveal",B()))};window.obFinishFromReveal=function(){if(!S)return;const e=ne[S.primary];Et(S.primary,S.secondary||"",e.weights,S.harmonySensitivity)};function Mt(e){const t={};Object.keys(ne).forEach(n=>t[n]=0),e[0]==="A"&&(t.Formalist+=2,t.Sensualist+=1,t.Completionist+=1),e[0]==="C"&&(t.Visceralist+=2,t.Atmospherist+=1),e[0]==="D"&&(t.Revisionist+=3),e[0]==="B"&&(t.Narrativist+=1,t.Humanist+=1),e[1]==="A"&&(t.Absolutist+=3,t.Narrativist+=2),e[1]==="C"&&(t.Visceralist+=2,t.Atmospherist+=2),e[1]==="D"&&(t.Completionist+=1,t.Revisionist+=1),e[1]==="B"&&(t.Humanist+=1,t.Formalist+=1),e[2]==="A"&&(t.Atmospherist+=3),e[2]==="C"&&(t.Formalist+=2,t.Absolutist+=2),e[2]==="D"&&(t.Completionist+=2,t.Revisionist-=1),e[2]==="B"&&(t.Narrativist+=1),e[3]==="A"&&(t.Atmospherist+=2,t.Revisionist+=2),e[3]==="C"&&(t.Completionist+=3),e[3]==="D"&&(t.Atmospherist+=1),e[3]==="B"&&(t.Sensualist+=1),e[4]==="A"&&(t.Humanist+=3,t.Visceralist+=1),e[4]==="D"&&(t.Sensualist+=3),e[4]==="C"&&(t.Formalist+=1,t.Completionist+=1),e[4]==="B"&&(t.Narrativist+=1,t.Absolutist+=1);let o=.3;e[5]==="A"&&(t.Visceralist+=1,o=0),e[5]==="C"&&(t.Absolutist+=1,o=1),e[5]==="B"&&(o=.4);const i=Object.entries(t).sort((n,a)=>a[1]-n[1]);return{primary:i[0][0],secondary:i[1][1]>0?i[1][0]:null,harmonySensitivity:o}}async function Et(e,t,o,i){const n=crypto.randomUUID(),a=S._slug||Q.toLowerCase().replace(/[^a-z0-9]/g,"-")+"-"+Math.floor(Math.random()*9e3+1e3);Z({id:n,username:a,display_name:Q,archetype:e,archetype_secondary:t,weights:o,harmony_sensitivity:i}),V(),oe(),document.getElementById("onboarding-overlay").style.display="none";const s=await M(()=>Promise.resolve().then(()=>F),void 0);s.updateMastheadProfile(),s.updateStorageStatus(),s.setCloudStatus("syncing"),A(),J(),Ie().catch(l=>console.warn("Initial sync failed:",l))}const St=Object.freeze(Object.defineProperty({__proto__:null,launchOnboarding:pe},Symbol.toStringTag,{value:"Module"})),le="f5a446a5f70a9f6a16a8ddd052c121f2",ce="https://api.themoviedb.org/3";let m={title:"",year:null,director:"",writer:"",cast:"",scores:{}},U=[],k={},q={};function De(e){G(e)}function G(e){for(let t=1;t<=4;t++){const o=document.getElementById("sn"+t),i=document.getElementById("sl"+t);t<e?(o.className="step-num done",o.textContent="✓"):t===e?(o.className="step-num active",o.textContent=t,i.className="step-label active"):(o.className="step-num",o.textContent=t,i.className="step-label")}document.querySelectorAll(".step-panel").forEach((t,o)=>{t.classList.toggle("active",o+1===e)})}let ge=null;function Ae(e){clearTimeout(ge);const t=document.getElementById("tmdb-results");if(e.trim().length<2){t.innerHTML="";return}document.getElementById("searchSpinner").style.display="inline",ge=setTimeout(async()=>{try{const i=await(await fetch(`${ce}/search/movie?api_key=${le}&query=${encodeURIComponent(e.trim())}&include_adult=false`)).json();if(document.getElementById("searchSpinner").style.display="none",!i.results||i.results.length===0){t.innerHTML='<div class="tmdb-loading">No results yet…</div>';return}const n=i.results.slice(0,6);t.innerHTML=n.map(a=>{const s=a.release_date?a.release_date.slice(0,4):"?",l=a.poster_path?`<img class="tmdb-result-poster" src="https://image.tmdb.org/t/p/w92${a.poster_path}" alt="">`:'<div class="tmdb-result-poster-placeholder">NO IMG</div>',r=(a.overview||"").slice(0,100)+((a.overview||"").length>100?"…":"");return`<div class="tmdb-result" onclick="tmdbSelect(${a.id}, '${a.title.replace(/'/g,"\\'").replace(/"/g,'\\"')}')">
          ${l}
          <div class="tmdb-result-info">
        ...
        </div>`}).join("")}catch{document.getElementById("searchSpinner").style.display="none",t.innerHTML='<div class="tmdb-error">Search failed — check connection.</div>'}},280)}async function Le(e,t){document.getElementById("tmdb-results").innerHTML='<div class="tmdb-loading">Loading film details…</div>';try{const[o,i]=await Promise.all([fetch(`${ce}/movie/${e}?api_key=${le}`),fetch(`${ce}/movie/${e}/credits?api_key=${le}`)]),n=await o.json(),a=await i.json(),s=n.release_date?parseInt(n.release_date.slice(0,4)):null,l=n.poster_path?`https://image.tmdb.org/t/p/w185${n.poster_path}`:null,r=a.crew.filter(f=>f.job==="Director").map(f=>f.name),c=a.crew.filter(f=>["Screenplay","Writer","Story","Original Story","Novel"].includes(f.job)).map(f=>f.name).filter((f,E,I)=>I.indexOf(f)===E),p=a.cast||[],d=p.slice(0,8);U=p;const v=n.production_companies||[];m._tmdbId=e,m._tmdbDetail=n,m.year=s,m._allDirectors=r,m._allWriters=c,m._posterUrl=l,k={},d.forEach(f=>{k[f.id]={actor:f,checked:!0}}),q={},v.forEach(f=>{q[f.id]={company:f,checked:!0}}),document.getElementById("tmdb-film-header").innerHTML=`
  ...
  </div>`}function ze(e){k[e]&&(k[e].checked=!k[e].checked);const t=document.getElementById("castItem_"+e),o=k[e].checked;t.className="cast-item "+(o?"checked":"unchecked"),t.querySelector(".cast-check").textContent=o?"✓":""}async function Pe(){const e=document.getElementById("moreCastBtn");e.textContent="Loading…",e.disabled=!0,U.slice(8,20).forEach(i=>{k[i.id]||(k[i.id]={actor:i,checked:!1})});const o=U.slice(0,20);je(o),e.textContent="+ More cast",e.disabled=!1,U.length<=20&&(e.style.display="none")}function It(e){document.getElementById("curate-companies").innerHTML=`<div class="company-chips">
    ${e.map(t=>`
  ...
  </div>`}function qe(e){q[e].checked=!q[e].checked;const t=document.getElementById("companyChip_"+e);t.className="company-chip "+(q[e].checked?"checked":"unchecked")}function Oe(){document.getElementById("tmdb-search-phase").style.display="block",document.getElementById("tmdb-curation-phase").style.display="none",document.getElementById("tmdb-results").innerHTML=""}function Re(){const e=m._allDirectors||[],t=m._allWriters||[],o=Object.values(k).filter(n=>n.checked).map(n=>n.actor.name),i=Object.values(q).filter(n=>n.checked).map(n=>n.company.name);m.title=m._tmdbDetail.title,m.director=e.join(", "),m.writer=t.join(", "),m.cast=o.join(", "),m.productionCompanies=i.join(", "),Ct(),G(2)}function _t(e){const t=[...u].filter(i=>i.scores[e]!=null).sort((i,n)=>n.scores[e]-i.scores[e]),o=t.length;return[t[Math.floor(o*.05)],t[Math.floor(o*.25)],t[Math.floor(o*.5)],t[Math.floor(o*.75)],t[Math.floor(o*.95)]].filter(Boolean)}function Ct(){const e=document.getElementById("calibrationCategories");e.innerHTML=x.map(t=>{const o=_t(t.key),i=m.scores[t.key]||75;return`<div class="category-section" id="catSection_${t.key}">
      ...
      <div class="slider-section">
        <div class="slider-label-row">
          ...
          <div>
            ...
            <span class="slider-desc" id="sliderDesc_${t.key}" style="margin-left:8px">${C(i)}</span>
          </div>
        </div>
    ...
    </div>`}).join(""),x.forEach(t=>{m.scores[t.key]||(m.scores[t.key]=75)})}window.selectAnchor=function(e,t,o){o.closest(".anchor-row").querySelectorAll(".anchor-film").forEach(a=>a.classList.remove("selected")),o.classList.add("selected");const i=m.scores[e]||75,n=Math.round((i+t)/2);document.getElementById("slider_"+e).value=n,updateSlider(e,n)};window.updateSlider=function(e,t){t=parseInt(t),m.scores[e]=t,document.getElementById("sliderVal_"+e).textContent=t,document.getElementById("sliderDesc_"+e).textContent=C(t)};function Ne(){Bt(),G(3)}let D=[],N=0;function Bt(){D=[],x.forEach(e=>{const t=m.scores[e.key];if(!t)return;u.filter(i=>i.scores[e.key]!=null&&Math.abs(i.scores[e.key]-t)<=3).sort((i,n)=>Math.abs(i.scores[e.key]-t)-Math.abs(n.scores[e.key]-t)).slice(0,1).forEach(i=>D.push({cat:e,film:i}))}),D=D.slice(0,6),N=0,me()}function me(){const e=document.getElementById("hthContainer");if(D.length===0||N>=D.length){e.innerHTML=`<div style="text-align:center;padding:40px;color:var(--dim);font-style:italic">
      No close comparisons needed — your scores are clearly differentiated. Click Continue.
    </div>`;return}const{cat:t,film:o}=D[N],i=m.scores[t.key];e.innerHTML=`
    <div style="font-family:'DM Mono',monospace;font-size:10px;color:var(--dim);text-transform:uppercase;letter-spacing:1.5px;margin-bottom:6px">
      Comparison ${N+1} of ${D.length} &nbsp;·&nbsp; ${t.label} (×${t.weight})
    </div>
    ...
    <div class="hth-row">
      <div class="hth-card" onclick="hthChoice('new', '${t.key}', ${o.scores[t.key]})">
        ...
        <div style="font-family:'DM Mono',monospace;font-size:10px;color:var(--dim);margin-top:4px">${C(i)}</div>
      </div>
      <div class="hth-vs">vs</div>
      <div class="hth-card" onclick="hthChoice('existing', '${t.key}', ${o.scores[t.key]})">
        ...
        <div style="font-family:'DM Mono',monospace;font-size:10px;color:var(--dim);margin-top:4px">${C(o.scores[t.key])}</div>
      </div>
    </div>
    <div class="hth-skip" onclick="hthSkip()">They're equal / skip this one</div>
  `}window.hthChoice=function(e,t,o){const i=m.scores[t];e==="new"&&i<=o?m.scores[t]=o+1:e==="existing"&&i>=o&&(m.scores[t]=o-1),N++,me()};window.hthSkip=function(){N++,me()};function He(){Tt(),G(4)}function Tt(){const e=te(m.scores);m.total=e;const t=[...u,m].sort((i,n)=>n.total-i.total),o=t.indexOf(m)+1;document.getElementById("resultCard").innerHTML=`
    <div style="font-family:'DM Mono',monospace;font-size:10px;color:var(--dim);text-transform:uppercase;letter-spacing:1.5px;margin-bottom:6px">
    ...
    <div class="result-label">${C(e)}</div>
    <div class="result-grid">
      ${x.map(i=>`
        <div class="result-cat">
          <div class="result-cat-name">${i.label} ×${i.weight}</div>
          <div class="result-cat-val ${z(m.scores[i.key]||0)}">${m.scores[i.key]||"—"}</div>
        </div>`).join("")}
    </div>
  ...
  `}function Fe(){m.total=te(m.scores),u.push({title:m.title,year:m.year,total:m.total,director:m.director,writer:m.writer,cast:m.cast,productionCompanies:m.productionCompanies||"",poster:m._tmdbDetail?.poster_path||null,overview:m._tmdbDetail?.overview||"",scores:{...m.scores}}),H(),M(()=>Promise.resolve().then(()=>F),void 0).then(e=>e.updateStorageStatus()),m={title:"",year:null,director:"",writer:"",cast:"",productionCompanies:"",scores:{}},k={},q={},U=[],document.getElementById("f-search").value="",document.getElementById("tmdb-results").innerHTML="",document.getElementById("tmdb-search-phase").style.display="block",document.getElementById("tmdb-curation-phase").style.display="none",document.getElementById("moreCastBtn").style.display="",G(1),A(),document.querySelectorAll(".screen").forEach(e=>e.classList.remove("active")),document.getElementById("rankings").classList.add("active"),document.querySelectorAll(".nav-btn").forEach(e=>e.classList.remove("active")),document.querySelectorAll(".nav-btn")[0].classList.add("active")}const Dt=Object.freeze(Object.defineProperty({__proto__:null,confirmTmdbData:Re,goToStep:De,goToStep3:Ne,goToStep4:He,liveSearch:Ae,resetToSearch:Oe,saveFilm:Fe,showMoreCast:Pe,tmdbSelect:Le,toggleCast:ze,toggleCompany:qe},Symbol.toStringTag,{value:"Module"}));function At(){if(!g){M(()=>Promise.resolve().then(()=>St),void 0).then(e=>e.launchOnboarding());return}We()}function We(){if(!g)return;const e=g.weights||{},t=Math.max(...Object.values(e));document.getElementById("archetypeModalContent").innerHTML=`
    ...
    <div style="font-family:'Playfair Display',serif;font-style:italic;font-size:32px;font-weight:900;color:var(--blue);margin-bottom:4px">${g.archetype||"—"}</div>
    ${g.archetype_secondary?`<div style="font-family:'DM Mono',monospace;font-size:11px;color:var(--dim);margin-bottom:4px">Secondary: ${g.archetype_secondary}</div>`:""}
    <div style="font-family:'DM Mono',monospace;font-size:10px;color:var(--dim);margin-bottom:28px">${g.username||""}</div>
  ...
  `,document.getElementById("archetypeModal").classList.add("open")}function Ue(e,t){const o=x.map(n=>({key:n.key,val:parseFloat(document.getElementById("awval_"+n.key)?.value)||1})),i=Math.max(...o.map(n=>n.val));o.forEach(n=>{const a=document.getElementById("awbar_"+n.key);a&&(a.style.width=Math.round(n.val/i*100)+"%")})}function Lt(){if(!g||!g.archetype)return;const e=ne[g.archetype]?.weights;e&&(x.forEach(t=>{const o=document.getElementById("awval_"+t.key);o&&(o.value=e[t.key]||1)}),Ue())}function jt(){const e={};x.forEach(t=>{const o=parseFloat(document.getElementById("awval_"+t.key)?.value);e[t.key]=isNaN(o)||o<1?1:Math.min(10,o)}),g.weights=e,M(()=>Promise.resolve().then(()=>Be),void 0).then(t=>t.saveUserLocally()),V(),A(),H(),Ye()}function Ye(e){(!e||e.target===document.getElementById("archetypeModal"))&&document.getElementById("archetypeModal").classList.remove("open")}function Ve(e){document.querySelectorAll(".screen").forEach(t=>t.classList.remove("active")),document.getElementById(e).classList.add("active"),document.querySelectorAll(".nav-btn").forEach(t=>t.classList.remove("active")),event.target.classList.add("active"),e==="analysis"&&we(),e==="calibration"&&de(),e==="explore"&&ie(),e==="predict"&&$e(),localStorage.setItem("ledger_last_screen",e)}function ue(){const e=document.getElementById("storageStatus");e&&(u.length>0?(e.textContent=`✓ ${u.length} films · saved`,e.style.color="var(--green)"):(e.textContent="no films yet",e.style.color="var(--dim)"))}function ye(){const e=g;if(!e)return;const t=document.getElementById("mastheadLeft");t.innerHTML=`<span class="profile-chip" onclick="window.__ledger.showSyncPanel()">
    <span style="font-size:9px">▾</span>
    <strong style="color:var(--ink)">${e.display_name}</strong>
    &nbsp;·&nbsp; ${e.archetype||"film watcher"}
  </span>`}function Je(){const e=new Blob([JSON.stringify(u,null,2)],{type:"application/json"}),t=document.createElement("a");t.href=URL.createObjectURL(e),t.download="film_rankings.json",t.click()}function Ge(){confirm("Clear all your films and start fresh? This cannot be undone.")&&(localStorage.removeItem("filmRankings_v1"),localStorage.removeItem("ledger_user"),location.reload())}async function zt(){ut(),Ce(),g?(Y("syncing"),ye(),V(),_e(g.id).catch(()=>Y("error"))):(Y("local"),setTimeout(()=>pe(),400)),A(),ue();const e=localStorage.getItem("ledger_last_screen");if(e&&e!=="rankings"&&document.getElementById(e)){const t=document.querySelectorAll(".nav-btn");t.forEach(o=>o.classList.remove("active")),document.querySelectorAll(".screen").forEach(o=>o.classList.remove("active")),document.getElementById(e).classList.add("active"),t.forEach(o=>{o.getAttribute("onclick")?.includes(e)&&o.classList.add("active")}),e==="analysis"&&we(),e==="explore"&&ie()}}function Y(e){const t=document.getElementById("cloudDot"),o=document.getElementById("cloudLabel");t.className="cloud-dot",e==="syncing"?(t.classList.add("syncing"),o.textContent="syncing…"):e==="synced"?(t.classList.add("synced"),o.textContent=g?g.display_name:"synced"):e==="error"?(t.classList.add("error"),o.textContent="offline"):o.textContent="local"}window.__ledger={showScreen:Ve,sortBy:be,openModal:Ze,closeModal:et,exploreEntity:ot,renderExploreIndex:ie,initPredict:$e,predictSearch:ke,predictSearchDebounce:at,predictSelectFilm:rt,predictAddToList:mt,startCalibration:gt,selectCalCat:ft,selectCalInt:vt,applyCalibration:xt,resetCalibration:de,launchOnboarding:pe,liveSearch:Ae,tmdbSelect:Le,toggleCast:ze,showMoreCast:Pe,toggleCompany:qe,resetToSearch:Oe,confirmTmdbData:Re,goToStep3:Ne,goToStep4:He,saveFilm:Fe,goToStep:De,showSyncPanel:At,openArchetypeModal:We,closeArchetypeModal:Ye,previewWeight:Ue,resetArchetypeWeights:Lt,saveArchetypeWeights:jt,exportData:Je,resetStorage:Ge,updateStorageStatus:ue,updateMastheadProfile:ye,setCloudStatus:Y};const Pt=["showScreen","sortBy","openModal","closeModal","exploreEntity","renderExploreIndex","initPredict","predictSearch","predictSearchDebounce","predictSelectFilm","predictAddToList","startCalibration","selectCalCat","selectCalInt","applyCalibration","resetCalibration","launchOnboarding","liveSearch","tmdbSelect","toggleCast","showMoreCast","toggleCompany","resetToSearch","confirmTmdbData","goToStep3","goToStep4","saveFilm","goToStep","showSyncPanel","openArchetypeModal","closeArchetypeModal","previewWeight","resetArchetypeWeights","saveArchetypeWeights","exportData","resetStorage"];Pt.forEach(e=>{window[e]=window.__ledger[e]});zt();const F=Object.freeze(Object.defineProperty({__proto__:null,exportData:Je,resetStorage:Ge,setCloudStatus:Y,showScreen:Ve,updateMastheadProfile:ye,updateStorageStatus:ue},Symbol.toStringTag,{value:"Module"}));
