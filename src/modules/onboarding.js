...
function renderTasteReveal() {
  ...
  const radarAxes = catKeys.map((c, i) => {
    ...
    return `<line x1="${cx}" y1="${cy}" x2="${ex}" y2="${ey}" stroke="rgba(12,11,9,0.08)" stroke-width="0.5"/>`;
  }).join('');
  const radarLabels = catKeys.map((c, i) => {
    ...
    return `<text x="${lx}" y="${ly}" text-anchor="${anchor}" dominant-baseline="central" font-family="'DM Mono',monospace" font-size="8" fill="rgba(12,11,9,0.4)">${label}</text>`;
  }).join('');
  ...
  const radarRings = [0.25, 0.5, 0.75, 1].map(level => {
    ...
    return `<polygon points="${pts}" fill="none" stroke="rgba(12,11,9,0.06)" stroke-width="0.5"/>`;
  }).join('');
